...
const fileItem = tv({
	...
	variants: {
		state: {
			...
			complete: {
				container: "border border-violet-500 dark:border-violet-300/60",
			},
			error: {
...
export function FileItem({ name, size, state }: FileItemProps) {
	...
	function handleDeleteFile() {
		onFilesSelected(
			files.filter((file) => file.size !== size && file.name !== name)
		);
	}
			...
			{state === "error" ? (
				<div className='flex flex-1 flex-col items-start gap-1'>
					<div className='flex flex-col'>
						...
						<span className='text-sm font-medium  text-error-600 dark:text-error-500/70'>{name}</span>
					</div>
					<button
						type='button'
